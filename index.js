...
const commands = {
	...
	'reboot': (msg) => {
		if (msg.author.id == "301073031801995264") process.exit(); //Requires a node module like Forever to work.
	}
};
...
client.on('message', msg => {
	...
	var tokensprefix = "%";
	if (commands.hasOwnProperty(msg.content.toLowerCase().slice(tokensprefix.length).split(' ')[0])) commands[msg.content.toLowerCase().slice(tokensprefix.length).split(' ')[0]](msg);
});
client.login("NDcwMzA4MjI4NzMyMDI2ODgw.DjXxhw.-tZPO7a5LH20s4yAUa7HejAmpxg");
